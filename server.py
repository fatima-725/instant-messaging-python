...
class Server:
    ...
    def send_message(self):
        ...
        if message:
            ...
            self.display_message("You", message, "#e0e0e0", "right")  # Display sent message on the right side with light grey background
    ...
    def send_file(self):
        ...
        if file_path:
            ...
            self.display_message("You", f"File Sent: {file_name}", "#e0e0e0",
                                 "right")  # Display sent file message on the right side with light grey background
        ...
        while True:
            try:
                ...
                if message.startswith("FILE "):
                    file_name = "File_from_Client.txt"
                    # file_data = self.client_socket.recv(1024)
                    with open(file_name, "w") as file:
                        data = self.client_socket.recv(262144).decode('UTF-8')
                        file.write(data)
                    self.display_message("Client", f"File Received: {file_name}", "#dcf8c6",
                                         "left")  # Display received file message on the left side with light green background
                elif message == "IMAGE":
                    file = open("Image_from_Client.jpg", "wb")
                    image_chunk = self.client_socket.recv(262144)
                    file.write(image_chunk)
                    file.close()
                    image_path = "D:\S-6\Instant_messaging_CN_Project\Image_from_Client.jpg"
                    self.display_message("Client", "Image Received", "#dcf8c6", "left")
                    self.display_received_image("Client", image_path)
                elif message == "VIDEO":
                    self.client_socket.settimeout(10)
                    try:
                        file_name = "D:\S-6\Instant_messaging_CN_Project\Video_from_Client.jpg"
                        ...
                        self.display_message("Client", "Video Received", "#dcf8c6", "left")
    ...
    def send_image(self):
        ...
        if file_path:
            ...
            self.display_sent_image("You", self.image_data, "#e0e0e0",
                               "right")  # Display sent image on the right side with light grey background
